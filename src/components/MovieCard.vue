<template>
    <div
        ...
        <div class="relative overflow-hidden">
            <img :src="'https://image.tmdb.org/t/p/original' + media.poster_path" :alt=media.title :key=media.id
        ...
        <div class="p-4">
            <h3 @click="getDetail(media.id, media_type)"
...
function getDetail(id: number, type: string) {
    if(type == 'tv'){
        router.push({ name: 'TvDetailPage', params: { type, id: id.toString() } });
    }else {
        router.push({ name: 'MovieDetailPage', params: { type, id: id.toString() } });
    }
