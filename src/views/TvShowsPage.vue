<template>
  <div class="container py-4 mx-auto">
    <p class="p-8 text-3xl">{{ $t("filterPage.popularTVShows") }}</p>
        ...
        <div class="w-full px-4 py-2 bg-white shadow-lg rounded-xl">
          ...
          <div class="pt-2" :class="{ hidden: !sortIsOpen }">
            ...
            >
              ...
              <option value="first_air_date.asc">{{ $t("filterPage.first_air_date_asc") }}</option>
              <option value="first_air_date.desc">
                {{ $t("filterPage.first_air_date_desc") }}
              </option>
              <option value="name.asc">{{ $t("filterPage.name_asc") }}</option>
              <option value="name.desc">{{ $t("filterPage.name_desc") }}</option>
            </select>
          </div>
        </div>
        <div class="w-full px-4 py-2 bg-white shadow-lg rounded-xl">
...
<script setup name="TvShowsPage" lang="ts">
...
const { fetchGenres, discoverTvShows } = useMediaDetail();

onMounted(async () => {
  genres.value = await fetchGenres("tv");
  if (route.query.page) {
    filterOptions.value = {
      ...filterOptions.value,
      ...route.query,
    ...
    const res = await discoverTvShows(filterOptions.value);
...
async function discoverTv() {
  ...
  const q = {
    ...
    "first_air_date.gte": date_from.value,
    "first_air_date.lte": date_to.value,
  };
  ...
  results.value = (await discoverTvShows(filterOptions.value)).results;
